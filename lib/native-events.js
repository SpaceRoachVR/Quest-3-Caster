...
// Must match the output_width/output_height of each entry in profile.c. This
// is the only list of supported profiles; native-build-foundation.test.js
// cross-checks it against profile.c so the two cannot drift apart.
const PROFILE_OUTPUTS = Object.freeze({
  obsLowLatency1080p60: { width: 1792, height: 1008 },
  obsStabilized1080p60: { width: 1920, height: 1080 },
  obsLowLatencySquareLeft1080p60: { width: 1080, height: 1080 },
  obsLowLatencySquareRight1080p60: { width: 1080, height: 1080 },
});
const PROFILE_IDS = new Set(Object.keys(PROFILE_OUTPUTS));
...
function validateNativeEvent(event) {
  ...
  if (event.type === 'ready') {
    ...
    const expected = PROFILE_OUTPUTS[event.effectiveProfile];
