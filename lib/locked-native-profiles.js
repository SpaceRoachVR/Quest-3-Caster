...
function buildLockedMicrophoneArguments(request, effectiveProfile) {
  ...
  const args = [
    ...
    '--audio-codec',
    'opus',
    // scrcpy defaults to 128 kbps, which smears a close microphone once the
    // wearer raises their voice over loud game audio -- the encoder runs out
    // of bits exactly when the signal gets hot and dense. Confirmed by ear
    // against an uncompressed capture of the same scene: 128 kbps is muddy
    // through gunfire, 256 kbps is indistinguishable from raw PCM.
    '--audio-bit-rate=256K',
  ];
