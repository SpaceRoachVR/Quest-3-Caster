...
function validateNativeExecutionResults(manifest, results) {
  ...
  if (
    ...
    || capabilities.openclAvailable !== true
    || typeof capabilities.gpu !== 'string'
    || !capabilities.gpu
...
function verifyNativeBundleDirectory(
  ...
  const nativeUnitTestsResult = runExecutable(
    path.join(snapshot.bundleDirectory, inspected.nativeUnitTestsPath),
    [],
    executionOptions,
  );
  const syntheticProbeResult = runExecutable(
  ...
  validateNativeExecutionResults(inspected.manifest, {
    client: clientResult,
